...
class ConvEScorer(nn.Module):
    ...
    def __init__(
        ...
        if self.emb_dim % self.emb_height or self.emb_dim % self.emb_width:
            raise Exception(
                (
                    ...
                    self.emb_dim, aspect_ratio, self.emb_width, self.emb_height
                )
            )
        ...
        conv_output_width = (
            (self.emb_width - self.filter_size + (2 * self.padding))
            / self.stride
        ) + 1
    ...
    def forward(self, s_emb, p_emb):
        ...
        s_emb_2d = s_emb[:, 1:].view(
            -1, 1, int(self.emb_height), int(self.emb_width)
        )
        p_emb_2d = p_emb[:, 1:].view(
            -1, 1, int(self.emb_height), int(self.emb_width)
        )
        ...
        out = self.non_linear(out)
        out = torch.cat(
            [torch.ones((batch_size, 1), device=out.device), out], -1
        )
        return out


class ConvE(nn.Module, KnowledgeGraphEmbedding):
    def __init__(
        self,
        num_entities,
        num_relations,
