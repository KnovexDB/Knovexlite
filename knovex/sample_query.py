...
parser.add_argument("--data_folder", type=str, default='data/KG/FB15K-237')
...
parser.add_argument("--sample_formula_scope", type=str, default='EFO3-small', choices=['EFO3-small','real_EFO1', 'EFOX_minimal', 'EFOX', 'EFO1ex'])
parser.add_argument("--sample_formula_list", type=list, default=list(range(0, 1)))
parser.add_argument("--start_index", type=int, default=0)
parser.add_argument("--end_index", type=int, default=20)
parser.add_argument("--max_ans", type=int, default=100)
parser.add_argument("--store_each", type=int, default=20)
...
def sample_one_formula_query(given_lstr, part_kg: KnowledgeGraph, full_kg: KnowledgeGraph, num_samples, sample_mode,
    ...
    if num_samples == 0:
        return []
    fof = EFOQuery.from_lstr(given_lstr)
    free_variable_list = [term for term in fof.term_dict if 'f' in  term]
    ...
    with tqdm.tqdm(total=num_samples) as pbar:
        while pbar.n < num_samples:
            ...
            if qa_dict and str(qa_dict) not in stored_qa_dict:  # We notice sampling may fail and return None
                ...
                fof.formula.append_interpretation(qa_dict)
                now_index += 1
                if sample_mode == 'train':
                    if full_answer is None and len(fof.formula_list) == 1:
                        full_answer = fof.formula_list[0].deterministic_query_set_with_initialization(
                            now_index, full_kg, None, False, epfo_constraint)
                    elif len(fof.formula_list) != 1:
                        full_answer = fof.deterministic_query(now_index, full_kg, 'set')
                    easy_answer = set()
                else:
                    if isinstance(fof.formula, Conjunction):
                        full_answer = fof.deterministic_query(now_index, full_kg, 'set')
                        easy_answer = fof.deterministic_query(now_index, part_kg, 'set')
                    else:
...
if __name__ == "__main__":
    ...
    elif args.sample_formula_scope == 'EFO3-small':
        formula_scope = pd.read_csv(osp.join('data', 'DNF_EFO3_small_14_final.csv'))
    elif args.sample_formula_scope == 'real_EFO1':
        formula_scope = index2newlstr
    elif args.sample_formula_scope == 'EFO1ex':
        formula_scope = pd.read_csv(osp.join('data', 'DNF_evaluate_EFO1.csv'))
    else:
        ...
        if args.mode == 'train':
            ...
            if '!' in lstr:
                for j in range(0, args.num_negative - useful_num, args.store_each):
                    if args.inductive:
                        all_query, all_qa_dict = sample_one_formula_query(lstr, test_inf_kg, test_kg, args.store_each,
                                                                         args.mode,
                                                                         args.meaningful_negation, args.double_check,
                                                                         args.negation_tolerance,
                                                                         use_full_matrix, args.ncpus, args.max_ans,
                ...
                for j in range(0, args.num_positive - useful_num, args.store_each):
                    if args.inductive:
                        all_query, all_qa_dict = sample_one_formula_query(
                            lstr, test_inf_kg, test_kg, args.store_each, args.mode, args.meaningful_negation,
